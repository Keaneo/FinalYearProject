...
### Time conditions
dt         = 0.5 # Time step (ms)
time_wind  = int(5/dt) # Temporal window size for averaging
T_total    = int(5000/dt + time_wind) # Total number of steps

slide_wind = int(20/dt) # Sliding step for window
...
df = pd.DataFrame([r1_traj[w], r2_traj[w]], index=['r1', 'r2'])
df = df.transpose()
#df.to_csv('r1r2.csv', index=True)
#plt.xlim(left=0)
plt.xlabel('Time (ms)')
